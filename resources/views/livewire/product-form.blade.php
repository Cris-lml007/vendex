...
<div>
    <form wire:submit="save">
        <div class="modal-body">
            @if ($settings->serialized_products || $product->is_serialize)
                <div class="row mb-3">
                    <div class="col">
                        ...
                        <div class="input-group">
                            @if ($edit)
                                <div class="input-group-text">
                                    Bs({{ Number::format($product->kardex()?->first()?->exchange_rate?->usd_to_bs ?? 0, 2) }})
                                </div>
                            @else
                                <div class="input-group-text">
            ...
            @for ($i = 0; $i < $number_labels; $i++)
                <div class="row mb-3">
                    <div class="col">
                        <input type="text" class="form-control" placeholder="Ingrese Etiqueta"
                            wire:model="labels[{{ $i }}]">
                    </div>
                    <div class="col">
                    ...
                    <div class="col-1">
                        <button type="button" class="btn btn-danger" wire:click="removeTag({{ $i }})"><i
            ...
            <div class="row mb-3">
                <div class="col">
                    <button type="button" wire:click="addLabel" class="btn btn-primary w-100">Añadir
                        Etiqueta</button>
                </div>
            </div>
            ...
            <div class="row mb-3">
                <div class="col">
                    ...
                    <div class="input-group">
                        <input type="text" class="form-control" wire:model.live="barcode"
                            placeholder="Ingrese Barcode" @if ($edit) readonly @endif>
                        <button type="button" data-bs-target="#modal-scanner" data-bs-toggle="modal"
                            class="btn btn-primary input-group-text"
                            @if ($edit) disabled @endif><i class="fa fa-qrcode"></i></button>
                    </div>
                </div>
            </div>
            <div class="row mb-3">
                ...
                <div class="row mb-3">
                    <div class="col">
                        ...
                        <input type="number" class="form-control mb-3" placeholder="Ingrese Cantidad"
                            wire:model="tags">
                        <div class="d-flex justify-content-center mb-3">
                            <button type="button" class="btn btn-success" wire:click="generatePdf()">Generar
                                Etiquetas</button>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="col">
                            <table class="table table-striped">
                                ...
                                <tbody>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <input
                                                                    ...
                                                                    type="number" @class([
                                                                        'form-control',
                                                                        'bg-warning' => $min_quantity[$item->id] >= $stocks[$item->id] ?? 0,
                                                                    ])
                                                ...
                                                <tbody>
                                                    @foreach ($product->children()->where('is_serialize', false)->where('status', \App\Enums\Status::ACTIVE)->get() as $item)
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <a href="{{ route('admin.product.id', $item->id) }}"
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <select class="form-select"
                                                                    wire:model="product_serials[{{ $item->store_id }}]">
                                    ...
                                    <tr>
                                        <td><strong>SUBTOTAL</strong></td>
                                        <td>{{ $this->product->children()->where('is_serialize', true)->where('status', \App\Enums\Status::ACTIVE)->whereNotExists(function ($query) {
                                                $query->select(DB::raw(1))->from('detail_transactions')->where('product_id', $this->product->id);
                                            })->count() }}
                                        </td>
                                    </tr>
                                </tbody>
                                <tfoot>
                                    <th>TOTAL</th>
                                    <th>{{ $total +
                                        $this->product->children()->where('is_serialize', true)->where('status', \App\Enums\Status::ACTIVE)->whereNotExists(function ($query) {
                                                $query->select(DB::raw(1))->from('detail_transactions')->where('product_id', $this->product->id);
                                            })->count() }}
                                    </th>
                                </tfoot>
