                        ...
                        <livewire:table :heads="$heads1" :searchable="false">
                            @foreach ($sales ?? [] as $item)
                                <tr>
                                    ...
                                    <td>{{ $item->quantity }}</td>
                            <td>{{ Number::format($item->price* ($settings->currency_main == App\Enums\Currency::BS ? $item->exchange_rate->usd_to_bs : 1), 2) }}</td>
                                ...
                                <th colspan="2">{{ Number::format($sales->sum("price")* ($settings->currency_main == App\Enums\Currency::BS ? $item->exchange_rate->usd_to_bs : 1), 2) }} {{($settings->currency_main == App\Enums\Currency::BS ? 'Bs' : 'Usd')}}</th>
                                </livewire:slot>
                        </livewire:table>
                    </div>
                </div>
