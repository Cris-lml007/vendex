...
<body>
    ...
    @php
        if (\Illuminate\Support\Facades\Storage::disk('local')->exists('stores/' . $transaction->store_id . '.jpg')) {
            $url = base64_encode(\Illuminate\Support\Facades\Storage::disk('local')->get('stores/' . $transaction->store_id . '.jpg'));
        } else {
            $url = '';
        }
    @endphp
    ...
    <table style="width:100%; border-bottom:1px solid #000; padding-bottom:10px;">
        <tr>
            ...
            <td style="width:30%; text-align:left; vertical-align:top;">
                @if ($url != '')
                    <img src="data:image/png;base64,{{ $url }}" style="height:70px;">
                @endif
            </td>
            <!-- Título -->
    ...
    <div class="literal">
        ...
        {{ strtoupper($format->format($entero)) }} CON {{ str_pad($decimal, 2, '0', STR_PAD_LEFT) }}/100 BOLIVIANOS
